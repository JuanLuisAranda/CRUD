...
<html>
    ...
    <body id="container">
        <%
          ...
          us.add(new String(request.getParameter("Password")));

          ResultSet resultado = s.executeQuery("SELECT * FROM Usuario");

          HashMap<String, String> m = new HashMap<String, String>();
          while (resultado.next()) {
            m.put(resultado.getString("NomUsuario"), resultado.getString("Password"));
          }
          if (m.containsKey(us.get(0))) {
            if (m.get(us.get(0)).equals(us.get(1))) {
              
                out.println("<script> window.location.replace('http://localhost:3306/Perrera/index.jsp');</script>");
